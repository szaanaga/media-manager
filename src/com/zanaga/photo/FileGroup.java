...
public class FileGroup implements Comparable<FileGroup> {
	...
	private static HashSet<String> fileNameSet = new HashSet<>();
	...
	public void logComplete() {
		...
		getFileSet().stream().map(file -> file.getName()).sorted().forEach(name -> {
			if(buffer.length() > 0) {
				buffer.append(";");
			}
			buffer.append(name);
	...
	public int compareTo(FileGroup o) {
		...
		if(ret == 0) {
			String f1 = getFileSet().stream().map(file -> file.getName()).sorted().findFirst().orElse(null);
			String f2 = o.getFileSet().stream().map(file -> file.getName()).sorted().findFirst().orElse(null);
